...
not an independent trust signal. Production publication is fixed to `CorniiDog/OPEMOS` and fails before
generating a plan when the checkout's `origin` does not normalize to that exact
project. The former `CorniiDog/open-gpu-kernel-modules-steamos-support` origin
is accepted only as GitHub's legacy redirect; emitted manifests and plans always
carry the canonical `CorniiDog/OPEMOS` identity.
