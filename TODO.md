...
* [x] Keep the SteamOS A/B guardian migration list outside the persistent
  ...
  while services and hooks remain in the persistent `/etc` root. Existing
  installs migrate only when the old upper directory contains exactly the
  known regular 0644 keep-list; altered bytes, wrong types or modes, symlinks,
  and additional data fail closed without mutation. Focused coverage proves
  migration, exact bytes and mode, repeat installation, absence from the
  `/var`-backed upper layer, and destination confinement. `python3
  tests/interstitial.py`, shell syntax, and diff hygiene
...
* [x] Define an additive single compiled-driver product tar that wraps the
...
* [x] 2026-09-06 user-authorized cross-repository merge governance:
  ...
  counterpart-primary scheduler/handoff fallback recorded by the authority.
  The completed synchronization is recorded in
  `docs/boundary-decision-2026-09-06.md`. Canonical-first
  ...
  and diff checks passed again. Core PR
  https://github.com/CorniiDog/OPEMOS/pull/29 squash-merged as
  `73e8d15c07671f3174f1a948d525e18db1084e5a`; EXE PR
  https://github.com/CorniiDog/OPEMOS.EXE/pull/53 squash-merged as
  `507e23cf848cde3c74390f7e6c41ba09f9084a15`. Both authorities are
  byte-identical at Git blob `2f8424a1df29fce2859126f7c42fd1885db8a425`
  and SHA-256
...
* [ ] Add a real phase-by-phase failure/cancellation matrix for pacman hooks,
  ...
  * [x] Add cancellation after recursive cleanup starts the first unmount. The
    ...
    still-recorded mounts, and emits a mandatory terminal cleanup record after
    verified release. The fixture requires monotonic zero-to-four progress plus
    exactly four completed unmounts without a false release. The
    ...
    leak, and exact restoration to the original no-compression policy.
    Serialized shell and Python syntax validation run through `heavy.sh`;
  ...
  * [x] Add cancellation after the GRUB configuration helper starts but before
    ...
    compressed-profile fixture requires complete userspace and module progress,
    terminal phase `bootloader_config` and reason `cancelled`, exactly one
    indeterminate GRUB record with no false completion, no depmod, initramfs, or
    installation-state progress, byte-for-byte preservation of the original
    GRUB configuration, complete four-mount cleanup, no temporary workspace
    leak, and exact restoration of the original compression policy. Serialized
  ...
  * [x] Add cancellation after the exact first module-copy process starts but
    ...
    launcher in test and production modes. The isolated compressed-profile
    fixture requires complete userspace counters, terminal phase
    `module_install` and reason `cancelled`, exactly zero of five installed
    modules, no later progress, complete four-mount cleanup, no temporary
    workspace leak, and exact restoration of the original compression policy.
    Serialized shell and Python syntax validation passed through `heavy.sh`;
    required integration validation is recorded with the GitHub PR because this
    host's unchanged `bsdtar` limitation blocks the monolithic suite before
    mutation.
  ...
  * [x] Add cancellation after the userspace package transaction has actually
    ...
    package counter with the authenticated package total, no verification,
    module, boot, initramfs, or state progress, complete four-mount cleanup,
    exact compression-policy restoration, and removal of all temporary
    workspaces. Serialized Python syntax validation passed through `heavy.sh`;
    required integration validation is recorded with the GitHub PR because this
    host's unchanged `bsdtar` limitation blocks the monolithic suite before
    mutation.
  ...
  * [x] Strengthen the immediate mkinitcpio child-failure boundary. The fixture
    ...
    complete four-mount cleanup, and exact compression-policy restoration.
    Required validation evidence is recorded with the GitHub PR because this
    host's unchanged `bsdtar` limitation blocks the monolithic integration
  ...
  * [x] Add an injected depmod status-66 failure after GRUB completion. The
    ...
    initramfs progress, and complete mount-cleanup counters. Python syntax and
    diff checks passed. The required local `heavy.sh` matrix stopped before
    mutation because this host lacks `bsdtar`; it reported the existing nominal
    userspace-package metadata error, so no local integration pass is claimed.
    PR checks on the declared GitHub runner provide the reachable integration
    validation and are recorded after completion.
...
* [x] Expose the Core-owned exact-target maintainer build/package/release dry-run
  ...
  * [x] Remove the remaining immutable-root dependency from the automatic
    ...
    filesystem, while rollback generations live in the Core-owned persistent
    `/var/lib` state tree and retention runs with the same administrative
